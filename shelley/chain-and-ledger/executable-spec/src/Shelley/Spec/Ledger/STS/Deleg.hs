  ...
  case c of
    ...
    DCertMir (MIRCert targetPot credCoinMap) -> do
      ...
      let (potAmount, instantaneousRewards) =
            if targetPot == ReservesMIR
              then (_reserves acnt, iRReserves $ _irwd ds)
              else (_treasury acnt, iRTreasury $ _irwd ds)
      ...
      if targetPot == ReservesMIR
        then pure $ ds {_irwd = (_irwd ds) {iRReserves = combinedMap}}
        else pure $ ds {_irwd = (_irwd ds) {iRTreasury = combinedMap}}
    DCertPool _ -> do
